...
- 데이터 관리
  - 서론
    - 도메인 데이터 모델링의 몇가지 친숙한 방법
    - 스프링 데이터 프로젝트에 레포지토리 사용해서 데이터 관리하는 방법
    - 마이크로 서비스에서 데이터 접근 어떻게 하는지?
  - 데이터 모델링
    - 책소개: `에반스-도메인 주도 설계`
    - 도메인 주도 설계를 통해서 해결해야 하고자 하는 문제는 복잡성
    - 최적의 데이터 저장을 위해 적합한 저장소를 선택하는 일이란?
      - `데이터의 사용성`과 `투자 대비 효율측면`에서 매우 중요하다.
      - `비즈니스 부문`과 `기술 부문`이 상호 간 동일한 내용에 대해 `서로 명확한 이해`를 할 수 있도록 `간소화`할 수 있는 방법을 살펴볼 필요 있음.
      - `비지니스 도메인`의 관계와 `엔티티`를 기술적으로 묘사하는 `저장소`
      - 비지니스 목표를 달성하기 위해 필요한 적절한 데이터 스토어 사용을 바탕으로 개발자가 작업해야 하는 `코드의 양을 최소화` 해야 한다. 
    - 관계형 데이터베이스 관리 시스템
      - 최근 SQL 데이터 베이스는 강력한 트랜잭션 취약
      - 관계모델은 이제 그래프가 최선이다.
      - SQL 데이터베이스가 다수의 읽기 트랜잭션을 처리할 수는 있지만, 이를 위해서는 비정규화와 고가용성을 위해 일관성을 일부 포기한 모델의 적용이 필요하다.
      - 현재 RDBMS를 사용하고 있다면 JPA구현을 이용한 하이버네이트가 최선
        - 도메인 모델에 더 집중할 수 있게 도와줌.
        - 조금더 편리한 진화를 위해 어느 정도 제어를 포기 해야함.
    - NoSQL
      - 특정 사용 사례의 요구를 해결하기 위해 최적화된 다양한 데이터 모델을 제공한다.
      - 특정 데이터 모델의 경계 지어진 컨텍스트의 분리가 필요한 마이크로서비스 구현에 장점으로 작요할 수 있다.
      - `폴리글랏 영속성: 마틴 파울러` 다양한 데이터베이스를 사용한 아키텍처를 설명할때 사용해서 대중화된 용어이다.
      - 특정 문제를 해결하기 위해 이전처럼 관계형 데이터베이스를 사용하는 대신, 특정 사용 사례에 최적화된 NoSQL을 사용할 수 있다. 
  - 스프링 데이터
      - 데이터베이스 모델의 `특수성`을 보존하면서도 `데이터 스토어`와 상호작용을 위한 포괄적인 추상화를 제공하는 `오픈소스 프로젝트`다.
      - 지원하는 데이터베이스 모듈 
        - JDBC, JPA, 몽고디비, 네오포제이, 레디스, 엘라스틱서치, 솔라, 젬파이어, 카산드라, 카우치베이스
      - 스프링 데이터 애플리케이션의 구조
        - 스프링 데이터를 통해 데이터 계층 접근에 필요한 디자인 패턴을 이해하는 데 많은 도움을 얻을 수 있다. 
      - 도메인 클래스
        ...
        @AllArgsConstructor
        @NoArgsConstructor
        public class User { // 클래스 이름은 도메인 모델의 개념(명사 또는 엔티티)을 표현한다.
            private Long id;
            private String firstName; // 비공개 필드는 네이티브 데이터 스토어 안의 데이터 객체 속성과 매핑된다. 
        ...
        - 엔티티 클래스
          - 비공개 필드로 구성됨.
          - 공개를 위해 setter/getter 사용.
          - 도메인 클래스는 애플리케이션 안의 데이터 스토어의 (데이터베이스의 테이블 같은) 데이터 객체와 엔티티의 매핑을 표현한다. 

      - 레포지토리 
        - 에릭에반스의 책인 `도메인주도설계`에서 처음 소개 됨 
        - CrudRepository: 구현체
      - 도메인 데이터를 위한 자바 패키지 구성

        ![imgUrl](http://martinfowler.com/bliki/images/boundedContext/sketch.png)
