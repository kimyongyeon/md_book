...
![](https://www.ibm.com/support/knowledgecenter/ko/SSHS8R_7.1.0/com.ibm.worklight.dev.doc/admin/Pictures/push_notification_jms_topology.jpg)  
...
즉 메시지를 보내는 `프로듀서`와 메시지를 가져다가 처리하는 `컨슈머`는 서로 직접 연결해서 메시지를 주고받는 것이 아니라, 메시지 브로커에 접속한다. 오래된 방식의 브로커들은 일반적으로 확장이 어렵지만, 레빗엠큐나 아파치 카프카 같은 도구들은 메시지 처리 규모에 따라 확장이 가능하다.
